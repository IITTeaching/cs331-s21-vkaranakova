...
class LinkedList:
    ...
    def __delitem__(self, idx):
        ...
        if(idx >= self.length):
            ...
            for _ in range(idx):
                cur = cur.next
            cur.prior.next, cur.next.prior = cur.next, cur.prior
            self.length -= 1
        ### END SOLUTION
    ...
    def cursor_move(self, offset):
        ...
        ### BEGIN SOLUTION
        if(offset < 0):
            if not self.cursor and self.length > 0:
                raise Exception("Cursor has not been set yet!")
            for i in range(0,-n):
    ...
    def cursor_delete(self):
        ...
        ### BEGIN SOLUTION
        if not self.cursor and not (self.cursor == self.head):
            raise Exception("Need a cell to insert after!")
